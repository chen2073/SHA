...
__doc__ = '''
  ...
  STEP 1:   Pad the message so that its length is an integral multiple of 512 bits(sha512 multiple of 1024), the block size.
  ...
  STEP 2:   Generate the MESSAGE SCHEDULE required for processing a 512-bit block of the input
            message.  The message schedule consists of 64 32-bit WORDS(sha512 64 64-bit words). The first 16 of these
            words are obtained directly from the 512-bit block(sha512 blocksize is 1024).  The rest of the words are
    ...
    def sha512(self):
        ...
        for n in range(0, bv4.length(), 1024):
            ...
            for i in range(16, 80):
                ...
                words[i] = BitVector(
                    intVal=(int(words[i - 16]) + int(sigma1) + int(words[i - 7]) + int(sigma0)) & 0xFFFFFFFFFFFFFFFF,
                    size=64)
            ...
            for i in range(80):
                ...
                t1 = BitVector(intVal=(int(h) + int(ch) + int(sum_e) + int(words[i]) + int(K_bv[i])) & 0xFFFFFFFFFFFFFFFF,
                               size=64)
...
if __name__ == '__main__':
    ...
    with open(sys.argv[1], 'r') as file_to_hash, open(sys.argv[2], 'w') as output:
        message = file_to_hash.read()
        hasher = SHA512(message=message)
        hashval_in_hex = hasher.sha512()
        output.write(hashval_in_hex)
        sys.stdout.writelines((hashval_in_hex, "\n"))

        import hashlib
        hash_object = hashlib.sha512(b'testtest123')
